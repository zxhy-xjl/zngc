...
public class LCLTextLineCodecDecoder implements ProtocolDecoder {
    ...
    private static String CONTEXT = LCLTextLineCodecDecoder.class.getName() + ".context";
    ...
    private void decodeNormal(Context ctx, IoBuffer in, ProtocolDecoderOutput out) throws CharacterCodingException {
        ...
        while (in.hasRemaining()) { // 变量解码的IoBuffer
            ...
            System.out.println(delimBuf.get(matchCount));
        ...
        in.position(oldPos);
        try {
            System.out.println(new String(in.buf().array(),"utf-8"));
